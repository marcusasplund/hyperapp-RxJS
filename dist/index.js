...
var AsyncAction = (function (_super) {
    __extends$6(AsyncAction, _super);
...
var DelaySubscriber = (function (_super) {
    __extends$5(DelaySubscriber, _super);
...
var delay$ = function (_) { return Observable_2.of(_).delay(600); };

var delayedClicks$ = (new Subject_2())
.concatMap(delay$);

var started = window.performance.now();
delayedClicks$.subscribe(function (add) {
  var time = window.performance.now() - started;
  console.log(("executing at " + (time.toFixed(2)) + " ms"));
  add();
});

var actions = {
  add: function (store) { return ({
    count: ++store.count
  }); },
  scheduleAdd: function scheduleAdd (store, _, actions) {
    delayedClicks$.next(actions.add);
  },
  scheduleSub: function scheduleSub (store, _, actions) {
    delayedClicks$.next(actions.sub);
  },
  sub: function (store) { return ({
    count: --store.count
  }); }
};
...
var HeaderSection = function () { return h( 'header', null,
    h( 'h2', null, "hyperapp RxJS " ),
    h( 'p', null,
      h( 'small', null,
        h( 'em', null, "click away, clicks will be captured and actions will be queued up, then executed with minimum 600ms between each" ), ' ',
        h( 'a', { href: 'https://github.com/marcusasplund/hyperapp-RxJS' }, "source")
      )
    ),
    h( 'hr', null )
  ); };
...
/* eslint-disable no-unused-vars */
var AddButton = function (ref) {
    var actions = ref.actions;

    return h( 'button', { class: 'button', onclick: actions.scheduleAdd }, "add one +");
};

/* eslint-disable no-unused-vars */
var SubButton = function (ref) {
    var actions = ref.actions;

    return h( 'button', { class: 'button', onclick: actions.scheduleSub }, "subtract one -");
};

/* eslint-disable no-unused-vars */
var ShowConsole = function () { return h( 'fieldset', { id: 'console-log-div' },
    logs.map(function (logitem) { return createLogNode(logitem); })
  ); };
...
var createLogNode = function (logitem) { return h( 'div', { class: logitem.type },
    logitem.message
  ); };

/* eslint-disable no-unused-vars */
var view = function (store, actions) { return h( 'div', { class: 'container' },
    h( HeaderSection, null ),
    h( AddButton, { actions: actions }),
    h( SubButton, { actions: actions }),
    h( ShowConsole, null )
  ); };
